...
plt.subplot(3,2,1)
file_dir = 'D:/Refresh/data/CHINARE-362/全水深CTD/'
...
for i in range(len(stationnames)):
    ...
    data_plt[i,:,0] = data_sum[i][:depth,28]    #温度
    data_plt[i,:,1] = data_sum[i][:depth,32]    #盐度
    data_plt[i,:,2] = data_sum[i][:depth,36]/210    #深度
    data_plt[i,:,3] = data_sum[i][:depth,24]    #密度
...
def plot(value_name,vmin,vmax,title):
    df_plt = pd.read_excel('D:/Refresh/data/CHINARE-362/水柱积分藻种数据.xlsx').set_index('Station')
    #导入数据
    ...
    #画底图
    lon_left,lon_right,lat_bottom,lat_top = -124,-109,-75,-65
    plt.gca().xaxis.set_ticks_position('top')
    support_pie.contourf_re(image,lon_left,lon_right,lat_bottom,lat_top)
    plt.text(lon_left+1,-66,title,fontsize=14)
    plt.text(lon_right-1,-66,support_pie.getChar(i+1),fontsize=14)

value = ['MLD(m)','%MW','mean_E','Zeu','PAR']
title = ['MLD/m','MW%','$\mathregular{E_{stability}}$','Zeu/m',
         'PAR/E·$\mathregular{m^{-2}}$·$\mathregular{day^{-1}}$']
value_min,value_max = [10,0,2,18,30],[40,7,12,78,50]
for i in range(5):
    plt.subplot(3,2,i+2)
    plot(value[i],value_min[i],value_max[i],title[i])
plt.savefig('D:/Refresh/data/CHINARE-362/pic/水文海洋学参数分布-英文.png'
