...
#    cb.set_ticks(np.round(np.linspace(vmin,vmax,10),1)) 
    ...
    plt.text(lon_left+0.5,-66,title,fontsize=14)
    plt.text(lon_right-1,-66,support_pie.getChar(i),fontsize=14)
...
value = ['Diat1_p', 'Diat2_p', 'Hapt6HiFe_p','Hapt6LoFe_p',
         'Dino1_p','Chloro_p', 'Crypto_p']
title = ['Diatoms-A/%','Diatoms-B/%','P. antarctica (high Fe)/%',
         'P. antarctica (low Fe)/%','Dinoflagellate/%','Chlorophytes/%','Cryptophytes/%']
value_min,value_max = [0,0,0,0,0,0,0],[75,75,75,75,75,75,75]
...
ax.set_xticks(())
ax.set_yticks(())
plt.savefig('D:/Refresh/data/CHINARE-362/pic/藻种水柱积分相对丰度平面分布-english.png'
